...
const SERVICE_SLUG = 'dhi-hair-transplant';
...
for (const locale of LOCALES) {
  ...
  await page.waitForSelector('#intro-overlay');
  await page.waitForFunction(() => document.querySelectorAll('.language-option').length === 8);
  await page.mouse.wheel(0, 1600);
  await page.waitForSelector('#intro-section', { state: 'hidden' });
  ...
  if (locale === 'en' || locale === 'ar') {
    await page.screenshot({
      path: path.join(ROOT, `.verify-${locale}-home.png`),
      fullPage: false,
    });
  }

  await page.goto(
    `${BASE_URL}/${locale}/service.html?slug=${SERVICE_SLUG}`,
  ...
  assert(service.lang === locale, `[${locale}] Service page lang is incorrect`);
  assert(service.dir === (locale === 'ar' ? 'rtl' : 'ltr'), `[${locale}] Service direction is incorrect`);
  assert(service.languageOptions === 8, `[${locale}] Service language menu is incomplete`);
  assert(service.heading, `[${locale}] Service heading is missing`);
  assert(service.canonical?.includes(`/${locale}/service.html?slug=${SERVICE_SLUG}`), `[${locale}] Canonical is incorrect`);
  assert(service.alternates === 9, `[${locale}] Expected 8 locale alternates and x-default`);
  assert(service.overflow <= 1, `[${locale}] Service page has horizontal overflow (${service.overflow}px)`);
  assert(!service.hasOverlay, `[${locale}] Service Vite error overlay is visible`);
  if (locale !== 'tr') {
    assert(service.heading !== 'DHI Saç Ekimi', `[${locale}] Turkish service text leaked`);
  }
}
...
await page.goto(
  `${BASE_URL}/en/service.html?slug=${SERVICE_SLUG}`,
  { waitUntil: 'domcontentloaded' },
);
await page.locator('.language-trigger').click();
await Promise.all([
  page.waitForURL(`**/de/service.html?slug=${SERVICE_SLUG}`),
  page.locator('.language-option[data-locale="de"]').click(),
]);
assert(page.url().includes(`/de/service.html?slug=${SERVICE_SLUG}`), 'Language switch did not preserve the service slug');

const mobilePage = await context.newPage();
await mobilePage.setViewportSize({ width: 390, height: 844 });
await mobilePage.goto(
  `${BASE_URL}/ar/service.html?slug=${SERVICE_SLUG}`,
  { waitUntil: 'domcontentloaded' },
);
...
const mobile = await mobilePage.evaluate(() => ({
  dir: document.documentElement.dir,
  menuOpen: document.querySelector('#nav-menu')?.classList.contains('active'),
  overflow: document.documentElement.scrollWidth - window.innerWidth,
}));
...
await mobilePage.waitForSelector('#intro-overlay');
await mobilePage.waitForFunction(() => document.querySelectorAll('.language-option').length === 8);
await mobilePage.mouse.wheel(0, 1600);
await mobilePage.waitForSelector('#intro-section', { state: 'hidden' });
...
await browser.close();
console.log('Browser verification passed for 8 home routes, 8 service routes, language persistence, and Arabic RTL.');
