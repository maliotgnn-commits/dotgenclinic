...
import { loadMedikalRdContent } from './medikal-rd-content.js';
import { detectMedikalRdLocale } from './medikal-rd-routes.js';
import {
  applySeoLinks,
  buildCategoryGroups,
  getCurrentLocale,
...
const locale = getCurrentLocale('medikal-rd');
const medikalLocale = pathLocale || locale;
const [catalog, uiDictionary, eyeContent, medikalContent] = await Promise.all([
  loadContentCatalog(locale),
  loadUiDictionary(locale),
  loadEyeHealthContent(locale),
  loadMedikalRdContent(medikalLocale),
]);
const { page } = medikalContent;
        ...
        <div class="container nav-container">
          ...
          <div class="nav-actions">
            <div class="nav-language-slot">
              ${renderLanguageSwitcher(locale, 'medikal-rd', uiDictionary)}
            </div>
...
function renderHero() {
  const heroImage = page.hero.image || '/images/medikal_rd/medikal_rd_hero.png';
  return `
    ...
    <nav class="mr-breadcrumb" aria-label="Breadcrumb">
      <div class="container">
        <ol>
          ...
          <li><a href="${argeLandingPath(medikalLocale)}">${escapeHtml(page.sectionName)}</a></li>
...
function renderPage() {
  ...
  applySeoLinks(medikalLocale, 'medikal-rd');
...
function bootstrapMedikalRdPage() {
  renderPage();
  initSkipLink();
  initCustomCursor();
