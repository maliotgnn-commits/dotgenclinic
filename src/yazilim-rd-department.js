...
import { loadYazilimRdContent } from './yazilim-rd-content.js';
import { detectYazilimRdLocale } from './yazilim-rd-routes.js';
import {
  applySeoLinks,
  buildCategoryGroups,
  getCurrentLocale,
...
const app = document.getElementById('yazilim-rd-app');
const pathLocale = detectYazilimRdLocale();
const locale = getCurrentLocale('yazilim-rd');
const yazilimLocale = pathLocale || locale;
const [catalog, uiDictionary, eyeContent, yazilimContent] = await Promise.all([
  loadContentCatalog(locale),
  loadUiDictionary(locale),
  loadEyeHealthContent(locale),
  loadYazilimRdContent(yazilimLocale),
]);
const { page } = yazilimContent;
        ...
        <div class="container nav-container">
          ...
          <div class="nav-actions">
            <div class="nav-language-slot">
              ${renderLanguageSwitcher(locale, 'yazilim-rd', uiDictionary)}
            </div>
    ...
    <nav class="mr-breadcrumb" aria-label="Breadcrumb">
      <div class="container">
        <ol>
          ...
          <li><a href="${argeLandingPath(yazilimLocale)}">${escapeHtml(page.sectionName)}</a></li>
...
function renderPage() {
  ...
  applySeoLinks(yazilimLocale, 'yazilim-rd');
...
function bootstrapYazilimRdPage() {
  renderPage();
  initSkipLink();
  initCustomCursor();
